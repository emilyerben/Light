...
# Replicator
The reason that nanotechnology is not already dominating the global market is a manufacturing problem. In chemical reactions, nanoparticles are still produced under laboratory conditions, which costs a lot of money and a lot of time. Perhaps the most promising approach to designing replicators that can produce nanobots is to assemlbe them in a kinematic process. First proposed by John von Neumann and later described by Eric Drexler, an assemlber could assemble any moclecule in a mechanical process. In contrast to chemosynthesis, mechanosynthesis is used, which allows for positioning specific atoms and can therefore produce any molecule, regardless of whether it is chemically reproducible or not. So it is possible to form completely new particles.
